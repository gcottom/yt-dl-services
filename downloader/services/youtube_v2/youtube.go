...
func (s *Service) Download(ctx context.Context, id string, useEmbedded bool) ([]byte, error) {
	...
	bestFormat := getBestAudioFormat(videoInfo.Formats)
	if bestFormat == nil {
	...
	if useEmbedded {
		stream, _, err = s.YTEmbeddedClient.GetStream(videoInfo, bestFormat)
	} else {
		stream, _, err = s.YTClient.GetStream(videoInfo, bestFormat)
	}
	if err != nil {
...
func (s *Service) GetPlaylistEntriesFromMusicAPI(ctx context.Context, playlistID string) ([]string, error) {
	...
	req, err := s.HTTPClient.CreateRequest(http.MethodGet, fmt.Sprintf("%s:%d/playlist?id=%s", s.Config.BaseURL, s.Config.Ports.MusicAPI, playlistID), nil)
	if err != nil {
...
func (s *Service) GetVideoInfo(ctx context.Context, videoID string, useEmbedded bool) (string, string, error) {
	...
	var err error
	if useEmbedded {
		video, err = s.YTEmbeddedClient.GetVideo(videoID)
	} else {
